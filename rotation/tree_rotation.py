...
from tools import is_backward, is_modifier, is_post_modifier, is_pure_forward
from tqdm import tqdm

from depccg.cat import Category
from depccg.grammar.ja import (
...
order_to_forwardstring: dict[int, str] = {0: "fa", 1: "fc", 2: "fc", 3: "fc"}

order_to_forwardsymbol: dict[int, str] = {0: ">", 1: ">B", 2: ">B2", 3: ">B3"}

order_to_forwardcrossedstring: dict[int, str] = {1: "fx", 2: "fx", 3: "fx", 4: "fx"}
...
def unification(
    cat_symbol: str, left_cat: Category, right_cat: Category
) -> Optional[CombinatorResult]:
    match cat_symbol:
        case ">":
        ...
        case ">B2":
            ...
            if uni(left_cat, right_cat):
                result = (
                    right_cat
                    if is_modifier(left_cat)
                    else left_cat.functor(uni["a"] / uni["c"], uni["d"])
                )
...
def toLeftward(top: Tree) -> Tree:
    if not top.is_unary:
        ...
        def rebuild(
            ...
            Tree
        ]:  # When a input tree does not satisfy rotation conditions, always returns 'None'.
            if (not r.is_unary) and (
                r.op_symbol != "SSEQ"
            ):  # if node is binary and is not conjunction,
                y = cat_to_order[r.op_symbol]
                b, c = r.children
                if (
                    is_pure_forward(top.op_symbol)
                    and is_pure_forward(r.op_symbol)
                    and x >= y
                ):  # 1
                    new_leftorder = x - y + 1
                    new_toporder = y
                    ...
                    if isinstance(newleft, Tree):
                        return Tree.make_binary(
                            top.cat, newleft, c, r.op_string, r.op_symbol
                        )
                    else:
                    ...
                    is_backward(top.op_symbol) and is_backward(r.op_symbol) and y >= 1
                ):  # 2
                    new_leftorder = x
                    new_toporder = x + y - 1
                    newleft = rebuild(new_leftorder, b)
                    ...
                    is_modifier(a.cat)
                    and is_post_modifier(c.cat)
                    and a.cat.right == c.cat.left
                ):  # 3-1
                    new_leftorder = x
                    new_toporder = x
                    newleft = rebuild(new_leftorder, b)
                    ...
                    is_modifier(a.cat)
                    and is_modifier(b.cat)
                    and a.cat.right == b.cat.left
                ):  # 3-2
                    new_leftorder = 1
                    new_toporder = x
                    newleft = rebuild(new_leftorder, b)
                    ...
                    if isinstance(newleft, Tree):
                        ...
                        if isinstance(newleft_result, CombinatorResult):
                            newleft_cat = newleft_result.cat
                            newtop_result = unification(
                                order_to_forwardsymbol[new_toporder], newleft_cat, c.cat
                            )
                            if isinstance(newtop_result, CombinatorResult):
                                newtop_cat = newtop_result.cat
                                return Tree.make_binary(
                    ...
                    is_post_modifier(c.cat)
                    and top.op_symbol == ">"
                    and a.cat.is_functor
                    and a.cat.left.is_atomic
                ):  # 3-4-1
                    new_leftorder = 0
                    new_toporder = 0
                    newleft = rebuild(new_leftorder, b)
        ...
        if top.op_symbol != "SSEQ":
            rebranch = rebuild(cat_to_order[top.op_symbol], right)
